...
class DeformUnfoldFunction3D(Function):
    ...
    def forward(ctx,
        ...
        output = input.new_empty(
            DeformUnfoldFunction3D._output_size(input, ctx.kernel_size, ctx.padding,
                                                ctx.dilation, ctx.stride))
        ...
        if not input.is_cuda:
            raise NotImplementedError("Only CUDA is supported.")
        else:
            cur_im2col_step = min(ctx.im2col_step, input.shape[0])
            assert (input.shape[0] % cur_im2col_step) == 0, 'im2col step must divide batchsize'
            deform_conv_cuda.deform_unfold_forward_cuda_3d(
                input, offset, output, ctx.bufs_[0],
                ctx.kernel_size[2], ctx.kernel_size[1], ctx.kernel_size[0],
    ...
    def backward(ctx, grad_output):
        ...
        if not grad_output.is_cuda:
            ...
            if ctx.needs_input_grad[0] or ctx.needs_input_grad[1]:
                grad_input = torch.zeros_like(input)
                grad_offset = torch.zeros_like(offset)
                deform_conv_cuda.deform_unfold_backward_input_cuda_3d(
                    input, offset, grad_output, grad_input,
                    grad_offset, ctx.bufs_[0], ctx.kernel_size[2],
                    ctx.kernel_size[1], ctx.kernel_size[0], ctx.stride[2],
                    ctx.stride[1], ctx.stride[0], ctx.padding[2], ctx.padding[1],
                    ctx.padding[0], ctx.dilation[2], ctx.dilation[1],
                    ctx.dilation[0], ctx.deformable_groups,
                    cur_im2col_step)
    ...
    def _output_size(input, kernel_size, padding, dilation, stride):
        channels = input.size(1)
        img_output_size = (input.size(0), channels)
        for d in range(3):  # For depth, height, width
            in_size = input.size(d + 2)
            pad = padding[d]
            kernel = dilation[d] * (kernel_size[d] - 1) + 1
            stride_ = stride[d]
        ...
        if not all(map(lambda s: s > 0, img_output_size)):
            raise ValueError(
                "Convolution input is too small (output would be {})".format(
                    'x'.join(map(str, img_output_size))))
        output_size = (input.size(0), channels * kernel_size[0] * kernel_size[1] * kernel_size[2],
                       img_output_size[2] * img_output_size[3] * img_output_size[4])
        return output_size
    


deform_unfold = DeformUnfoldFunction.apply
deform_unfold_3D = DeformUnfoldFunction3D.apply
