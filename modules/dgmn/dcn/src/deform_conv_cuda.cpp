...
void unfold_shape_check_3d(at::Tensor input, at::Tensor offset,
                       int kD, int kH, int kW,
                       int dD, int dH, int dW, int padD, int padH, int padW,
                       int dilationD, int dilationH, int dilationW,
                       int deformable_group)
{
    AT_CHECK(kW > 0 && kH > 0 && kD > 0,
             "kernel size should be greater than zero, but got kD: %d kH: %d kW: %d",
             kD, kH, kW);

    AT_CHECK(dW > 0 && dH > 0 && dD > 0,
             "stride should be greater than zero, but got dD: %d dH: %d dW: %d", dD, dH, dW);

    AT_CHECK(dilationW > 0 && dilationH > 0 && dilationD > 0,
             "dilation should be greater than 0, but got dilationD: %d dilationH: %d dilationW: %d",
    ...
    AT_CHECK(ndim == 4 || ndim == 5,
             "4D or 5D input tensor expected but got: %dD", ndim);
    ...
    long inputWidth = input.size(dimw);
    long nOutputPlane = nInputPlane;
    long outputDepth = (inputDepth + 2 * padD - (dilationD * (kD - 1) + 1)) / dD + 1;
    long outputHeight = (inputHeight + 2 * padH - (dilationH * (kH - 1) + 1)) / dH + 1;
    long outputWidth = (inputWidth + 2 * padW - (dilationW * (kW - 1) + 1)) / dW + 1;
    ...
    AT_CHECK(input.size(1) == nInputPlane,
             "invalid number of input planes, expected: %ld, but got: %ld",
             nInputPlane, input.size(1));

    AT_CHECK((inputDepth >= kD && inputHeight >= kH && inputWidth >= kW),
             "input volume is smaller than kernel");

    AT_CHECK(
        (offset.size(2) == outputDepth && offset.size(3) == outputHeight && offset.size(4) == outputWidth),
        "invalid spatial size of offset, expected depth: %ld height: %ld width: %ld, but got depth: %ld height: %ld width: %ld",
        outputDepth, outputHeight, outputWidth, offset.size(2), offset.size(3), offset.size(4));

    AT_CHECK((offset.size(1) == deformable_group * 3 * kD * kH * kW),
             "invalid number of channels of offset");
}
...
int deform_unfold_forward_cuda_3d(at::Tensor input,
                              at::Tensor offset, at::Tensor output, 
                              at::Tensor columns, int kD, int kH, int kW,
                              int dD, int dH, int dW, int padD, int padH, int padW,
                              int dilationD, int dilationH, int dilationW, 
                              int deformable_group, int im2col_step)
{

    // Perform shape check for 3D tensors
    unfold_shape_check_3d(input, offset, kD, kH, kW, dD, dH, dW, padD, padH, padW, dilationD, dilationH, dilationW, deformable_group);
    ...
    // Check that batch size is divisible by im2col_step
    long batchSize = input.size(0);
    long nInputPlane = input.size(1);
    ...
    output = output.view({batchSize / im2col_step, im2col_step, nInputPlane * kD * kH * kW, outputDepth * outputHeight * outputWidth});
    columns = at::zeros({nInputPlane * kD * kH * kW, im2col_step * outputDepth * outputHeight * outputWidth}, input.type());

    input = input.view({batchSize / im2col_step, im2col_step, nInputPlane, inputDepth, inputHeight, inputWidth});
    offset = offset.view({batchSize / im2col_step, im2col_step,
                          deformable_group * 3 * kD * kH * kW, outputDepth, outputHeight, outputWidth});

    at::Tensor output_buffer = at::zeros({batchSize / im2col_step, nInputPlane * kD * kH * kW, im2col_step * outputDepth * outputHeight * outputWidth}, output.type());

    for (int elt = 0; elt < batchSize / im2col_step; elt++)
    {
        deformable_im2col_3d(
            input[elt], offset[elt], nInputPlane, inputDepth,
            inputHeight, inputWidth, kD, kH, kW, padD, padH, padW, dD, dH, dW, dilationD, dilationH, dilationW,
            im2col_step, deformable_group, columns);
        output_buffer[elt] = columns;
    }

    input = input.view({batchSize, nInputPlane, inputDepth, inputHeight, inputWidth});
    offset = offset.view({batchSize, deformable_group * 3 * kD * kH * kW, outputDepth, outputHeight, outputWidth});

    output_buffer = output_buffer.view(
        {batchSize / im2col_step, nInputPlane * kD * kH * kW, im2col_step, outputDepth * outputHeight * outputWidth});
    output_buffer.transpose_(1, 2);
    output.copy_(output_buffer);
    output = output.view({batchSize, nInputPlane * kD * kH * kW, outputDepth * outputHeight * outputWidth});

    if (batch == 0)
    {
        output = output.view({nOutputPlane * kD * kH * kW, outputDepth, outputHeight, outputWidth});
        input = input.view({nInputPlane, inputDepth, inputHeight, inputWidth});
...
int deform_unfold_backward_input_cuda_3d(
    at::Tensor input, at::Tensor offset, at::Tensor gradOutput,
    at::Tensor gradInput, at::Tensor gradOffset, 
    at::Tensor columns, int kD, int kH, int kW, int dD, int dH, int dW, int padD, int padH, int padW,
    int dilationD, int dilationH, int dilationW, int deformable_group, int im2col_step)
{
    unfold_shape_check_3d(input, offset, kD, kH, kW, dD, dH, dW, padD, padH, padW, dilationD, dilationH, dilationW, deformable_group);
    ...
    if (input.ndimension() == 4)
    {
        ...
        gradOutput = gradOutput.view({1, gradOutput.size(0), gradOutput.size(1), gradOutput.size(2), gradOutput.size(3)});
    }
    ...
    AT_CHECK((offset.size(0) == batchSize), "invalid batch size of offset");
    gradInput = gradInput.view({batchSize, nInputPlane, inputDepth, inputHeight, inputWidth});
    columns = at::zeros({nInputPlane * kD * kH * kW, im2col_step * outputDepth * outputHeight * outputWidth}, input.type());

    // Change order of grad output
    gradOutput = gradOutput.view(
        {batchSize / im2col_step, im2col_step, nInputPlane * kD * kH * kW, outputDepth * outputHeight * outputWidth});
    gradOutput.transpose_(1, 2);
    gradOutput = gradOutput.contiguous();
    gradOutput = gradOutput.view({batchSize / im2col_step, nInputPlane * kD * kH * kW, im2col_step * outputDepth * outputHeight * outputWidth});

    gradInput = gradInput.view(
        {batchSize / im2col_step, im2col_step, nInputPlane, inputDepth, inputHeight, inputWidth});
    input = input.view({batchSize / im2col_step, im2col_step, nInputPlane, inputDepth, inputHeight, inputWidth});
    gradOffset = gradOffset.view({batchSize / im2col_step, im2col_step,
                                  deformable_group * 3 * kD * kH * kW, outputDepth, outputHeight, outputWidth});
    offset = offset.view({batchSize / im2col_step, im2col_step,
    ...
    for (int elt = 0; elt < batchSize / im2col_step; elt++)
    {
        ...
        deformable_col2im_coord_3d(
            columns, input[elt], offset[elt],
            nInputPlane, inputDepth, inputHeight, inputWidth, kD, kH, kW, padD, padH, padW, dD, dH, dW,
            dilationD, dilationH, dilationW, im2col_step, deformable_group, gradOffset[elt]);

        deformable_col2im_3d(
            columns, offset[elt], nInputPlane, inputDepth,
            inputHeight, inputWidth, kD, kH, kW, padD, padH, padW, dD, dH, dW, dilationD, dilationH, dilationW, im2col_step,
            deformable_group, gradInput[elt]);
    }

    gradOutput.transpose_(1, 2);
    gradOutput = gradOutput.contiguous();
    gradOutput = gradOutput.view({batchSize, nInputPlane * kD * kH * kW, outputDepth * outputHeight * outputWidth});

    gradInput = gradInput.view({batchSize, nInputPlane, inputDepth, inputHeight, inputWidth});
    input = input.view({batchSize, nInputPlane, inputDepth, inputHeight, inputWidth});
    gradOffset = gradOffset.view({batchSize, deformable_group * 3 * kD * kH * kW, outputDepth, outputHeight, outputWidth});
    offset = offset.view({batchSize, deformable_group * 3 * kD * kH * kW, outputDepth, outputHeight, outputWidth});

    if (batch == 0)
    {
        gradOutput = gradOutput.view({nOutputPlane, outputDepth, outputHeight, outputWidth});
        input = input.view({nInputPlane, inputDepth, inputHeight, inputWidth});
        gradInput = gradInput.view({nInputPlane, inputDepth, inputHeight, inputWidth});
        offset = offset.view({offset.size(1), offset.size(2), offset.size(3), offset.size(4)});
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m)
{
    ...
    m.def("deform_unfold_forward_cuda_3d", &deform_unfold_forward_cuda_3d, "3d deform unfold forward (CUDA)");
    m.def("deform_unfold_backward_input_cuda_3d", &deform_unfold_backward_input_cuda_3d,
          "3d deform_unfold_backward_input (CUDA)");
}