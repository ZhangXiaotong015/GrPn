...
from datasets.dataset import Dataset_MSD_test
...
parser.add_argument('--log', type=str, default='/home/xzhang2/data1/liver_couinaud_segmentation/model/GRCNN/outputs', help='Path to model.')
...
class MSDConfig(Config):
    ...
    saving = True
    saving_path = '/home/data1/liver_couinaud_segmentation/model/GRCNN/outputs'
    saving_ply = False
...
def main():
    ...
    # Initiate dataset
    test_dataset = Dataset_MSD_test(config, name='testing')

    # Data loader
    test_loader = DataLoader(test_dataset,
                            batch_size=config.batch_size,
                            num_workers=0,#config.input_threads,
                            collate_fn=MSD_collate_test,
                            pin_memory=True
                            )
...
class ModelTester:
    ...
    def cloud_segmentation_test(self, net, test_loader, config, num_votes=100, debug=False):
        ...
        while True:
            ...
            for sub_i in range(len(batch)):
                ...
                with torch.no_grad():
                    ...
                    # outputs = net(inputs['points_0'],inputs['points_1'],inputs['points_2'],inputs['points_3'],inputs['points_4'],inputs['features'],
                    #               inputs['pools_0'],inputs['pools_1'],inputs['pools_2'],inputs['pools_3'],inputs['pools_4'],
                    #               inputs['neighbors_0'],inputs['neighbors_1'],inputs['neighbors_2'],inputs['neighbors_3'],inputs['neighbors_4'],
                    #               inputs['upsamples_0'],inputs['upsamples_1'],inputs['upsamples_2'],inputs['upsamples_3'],inputs['upsamples_4'])
                    # raise ValueError('Debug before FLOPs calculation!!!')
        ...
        # print(f'Average inference time per case: {total_time_ms / num_cases:.2f} ms')
        ...
        label_file = '/data1/liver_couinaud_segmentation/data/MSD_Couinaud/annotation/couinaud'
        image_nii_file = '/data1/liver_couinaud_segmentation/data/MSD_Couinaud/images'
    ...
    def points2img(self, Txt_file, save_file, label_file, image_nii_file):
        ...
        for file_name in Txt_folder:
            ...
            name = file_name.replace('.txt','.nii.gz')

            tem = name
            print("tem:", tem)
            label_name = name.replace('case_','hepaticvessel_')
            save_name = name.replace('case_','seg_')
            ...
            nii_path = os.path.join(label_file,name.replace('case_','hepaticvessel_'))
    ...
    def fill_holes_in_mask(self, pred_nii_file, save_fill_file, label_file):
        ...
        for file_name in pred_nii_folder:
            ...
            case_idx = str(int(re.findall(r"\d+",file_name)[0])).zfill(3)
            # label_name = f'case_{case_idx}.nii.gz'
            label_name = f'hepaticvessel_{case_idx}.nii.gz'
    ...
    def Resolution_Restoration(self, pred_fill_nii_file, save_final_file, label_file):
        ...
        for file_name in pred_nii_folder:
            ...
            case_idx = str(int(re.findall(r"\d+",file_name)[0])).zfill(3)
            label_name = f'hepaticvessel_{case_idx}.nii'
